...
본 저장소는 조선 후기 대문호 삼연(三淵) 김창흡(金昌翕)의 문집인 *삼연집(三淵集)*에 수록된 한시 **5,247편**을 대상으로 한 하이브리드 토픽 모델링 분석 코드와 데이터셋 일체를 제공한다.
...
🔗 **[https://drive.google.com/drive/folders/1r7aiWAzSzPrfo3x-GDULPWdWtGR4Ke1W?usp=sharing]**

> 최상위 공유 폴더명: `HDCTM-김창흡한시분석도구및결과`

> 💡 **원본성(날짜) 검증 안내**
>
> 분석 알고리즘이 구동되어 결과 파일들이 최초로 생성된 시점의 실제 타임스탬프(2026년 4월 20일 등 폴더의 속성 예외)를 그대로 복원하여 필학적 원본성을 검증하고자 하는 연구자는, 개별 파일 다운로드 대신 최상위 디렉터리에 배치된 `HDCTM_김창흡한시분석_통합패키지.zip` 파일을 다운로드하여 압축을 해제하는 것을 권장한다.
...
#### 최상위 경로 (Root)
#### 01_HDCTM_김창흡한시분석_통합패키지.zip 
- ** 소스 코드 및 데이터 전체를 포함한 일괄 압축 파일(데이터 투명성 검증용 원본 날짜 유지)
#### 02_하이브리드_토픽모델링_분석기.py`**: 대형 언어 모델(LLM)과 밀도 기반 군집화 알고리즘을 결합한 분석 파이프라인 구동용 통합 GUI 애플리케이션 소스 코드이다. 분석 초기 단계에 LLM을 가동하여 주제, 유형, 정서의 3축 키워드를 추출하고, 이를 한시 원문과 합성하여 '지식 증강형(Knowledge-Augmented) 임베딩' 공간을 구축하도록 설계되어 있다.

#### 03_논문 검증용 추출 데이터 (`01_Original_Data/`)

- **`삼연_시학_메타주제_표준화_통합데이터.xlsx`**: 데이터 기반의 하이브리드 토픽 모델링 분석 결과와 실제 인문학 논문 본문에 기술된 질적 비평 내용, 통계적 성과 수치 및 구체적인 서정 정조 사례 간의 교차 비교 검증을 위해 별도로 정제하여 확보한 원본 데이터셋이다.

#### 04_상향식 분석 결과 (`02_BottomUp_Analysis/`)
...
- **`step4_final_reports/`**: 설명 가능한 AI(XAI) 관점에서 기하학적 이상치(Outlier) 격리 근거 및 대표시-이웃 작품 간의 의미론적 일관성을 필로로지적으로 정성 검증할 수 있도록 지원하는 JSON 형식의 최종 리포트 분할 파일(0번~6번)들의 집합이다.

#### 05_하향식 병합 분석 결과 (`03_TopDown_Analysis/`)

데이터 제어 자유도를 최소화하여 의미론적 일관성의 붕괴를 제어하고, 규격화된 스키마 분류 템플릿에 따라 거시적 사상 정렬을 전개하는 하향식(Phase 2-A) 전문가 경로의 결과물이다. 실험 대상이 된 삼연집 한시 데이터 **5,247편 전체**를 완전히 내포하고 있다.
...
- **클라우드 API**: Gemini 1.5 Pro (Vertex AI) / 인코딩 설정을 보존하기 위해 모든 연산 파일은 **UTF-8-SIG** 규격을 적용함
...
### 데이터셋 및 도구 인용
[저자명], " HDCTM-김창흡 한시분석도구 및 결과", GitHub Repository, 2026.
...
- **소스 코드 (Python Script)**: [MIT License](https://opensource.org/licenses/MIT) 적용
- **데이터셋 (Excel, CSV, JSON, HTML)**: [Creative Commons Attribution 4.0 International (CC-BY-4.0)](https://creativecommons.org/licenses/by/4.0/) 라이선스에 의거하여, 적절한 출처 표기 및 인용 하에 상업적·비상업적 재배포와 2차 저작물 작성이 자유롭게 허용된다.
