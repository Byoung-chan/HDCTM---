환경이나 플랫폼에 따라 상단의 파일 다운로드 링크가 올바르게 표시되지 않거나 클릭이 불가능할 수 있습니다. 불편을 드려 죄송합니다.

이런 상황에서 가장 확실하고 간편하게 `README.md` 파일을 만드는 방법은 아래의 **텍스트를 그대로 복사하여 직접 컴퓨터에 저장**하는 것입니다.

아래 코드 블록의 내용을 복사해서 파일로 만드는 방법을 안내해 드립니다.

### 💾 README.md 파일 직접 만드는 방법

1. 아래 마크다운 소스 코드 블록 우측 상단의 **[복사(Copy)]** 버튼을 클릭합니다.
2. 컴퓨터에서 **메모장(Windows)** 또는 텍스트 편집기(Mac)를 엽니다.
3. 복사한 내용을 붙여넣기(**Ctrl + V** 또는 **Cmd + V**) 합니다.
4. 저장할 때 파일 이름을 반드시 **`README.md`** 로 입력합니다.
* **Windows 메모장 주의사항:** 저장 창이 뜨면 파일 형식을 `텍스트 문서(*.txt)`에서 `모든 파일 (*.*)`로 변경하고, 인코딩을 `UTF-8`로 선택한 뒤 저장하세요.



---

### 📝 README.md에 들어갈 마크다운 원본 소스 코드
